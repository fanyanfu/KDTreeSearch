function [idx, dist] = knnsearch1(obj, Y, varargin)
...
if numNN2 > 0
    if nargout < 2
        idx = knnsearchx(X2, Y', numNN2, minExp, obj, wasNaNY, includeTies);
    else
        [idx, dist]= knnsearchx(X2, Y',numNN2,minExp, obj, wasNaNY, includeTies);
    end
    if  ~includeTies
        idx =idx';
        if nargout > 1
